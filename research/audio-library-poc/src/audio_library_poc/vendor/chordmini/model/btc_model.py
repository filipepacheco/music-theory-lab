...
class BTC_model(nn.Module):
    ...
    def predict(
        self, x, per_frame=True, smooth=False, kernel_size=9, use_gaussian=False
    ):
        with torch.no_grad():
            logits = self(x)
            if smooth and logits.dim() >= 2:
                logits = self._apply_temporal_smoothing(
                    logits, kernel_size, use_gaussian
                )
            if logits.dim() == 3:
                if per_frame:
