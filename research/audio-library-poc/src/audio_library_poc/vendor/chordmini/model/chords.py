...
INTERVAL_SETS_TO_QUALITY = {
    ...
    frozenset({0, 7}): "maj",
    frozenset({0, 5, 7, 10}): "sus4",
    frozenset({0, 4, 8, 10}): "aug",
    frozenset({0, 2, 4, 7, 10}): "7",
    frozenset({0, 2, 3, 7, 10}): "min7",
...
def transpose_chord_label(chord_label, semitones):
    ...
    try:
        ...
        if len(root_upper) > 1:
            pitch_class += (
                1 if root_upper[1] == "#" else (-1 if root_upper[1] == "b" else 0)
            )
        new_pitch_class = (pitch_class + semitones) % 12
        new_root = PREFERRED_SPELLING_MAP.get(
            PITCH_CLASS[new_pitch_class], PITCH_CLASS[new_pitch_class]
        )

        new_bass = None
        if bass_part:
            if bass_part[0].isdigit() or (
                bass_part[0] in "#b" and len(bass_part) > 1 and bass_part[1].isdigit()
            ):
                new_bass = bass_part
            else:
                ...
                if bass_upper[0] in base_notes:
                    ...
                    if len(bass_upper) > 1:
                        bass_pitch_class += (
                            1
                            if bass_upper[1] == "#"
                            else (-1 if bass_upper[1] == "b" else 0)
                        )
                    new_bass_pc = (bass_pitch_class + semitones) % 12
                    new_bass = PREFERRED_SPELLING_MAP.get(
                        PITCH_CLASS[new_bass_pc], PITCH_CLASS[new_bass_pc]
                    )
...
class Chords:
    def __init__(self):
        self._shorthands = {
            "maj": self._ivl("(1,3,5)"),
            "min": self._ivl("(1,b3,5)"),
            "dim": self._ivl("(1,b3,b5)"),
            "aug": self._ivl("(1,3,#5)"),
            "maj7": self._ivl("(1,3,5,7)"),
            "min7": self._ivl("(1,b3,5,b7)"),
            "7": self._ivl("(1,3,5,b7)"),
            "5": self._ivl("(1,5)"),
            "dim7": self._ivl("(1,b3,b5,bb7)"),
            "hdim7": self._ivl("(1,b3,b5,b7)"),
            "minmaj7": self._ivl("(1,b3,5,7)"),
            "maj6": self._ivl("(1,3,5,6)"),
            "min6": self._ivl("(1,b3,5,6)"),
            "sus2": self._ivl("(1,2,5)"),
            "sus4": self._ivl("(1,4,5)"),
            "aug7": self._ivl("(1,3,#5,b7)"),
        }
        self.chord_mapping = {}
